...
export class LocalComponent implements OnInit {
  ...
  pushToCollection(where, data){
      ...
      if( where === this.allDeals  ){
          ...
          this.localDeals.map( data => { data.isBookmarked = true; } );
          this.localDeals = [...this.localDeals];
...
//   }
  ...
  getLocalDeals(){
      this.utils.getLocalDeals().subscribe(res=>{
            this.savedAreEmpty = false;
            this.pushToCollection( this.localDeals, res);
          },
          err =>{
            this.utils.openSnackBar('Sorry we could not get your saved deals, please try again');
          }
